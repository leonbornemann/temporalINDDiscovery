...
class SimpleShiftedTemporalINDTest extends AnyFlatSpec{
  ...
  "Different Validation Variants Tested" should "work correctly" in {
    ...
    var simpleRelaxedTemporalIND = new SimpleTimeWindowTemporalIND(history1, history2, 2, false, ValidationVariant.FULL_TIME_PERIOD)
    assert(simpleRelaxedTemporalIND.absoluteViolationTime == 3+2+2)
    assert(simpleRelaxedTemporalIND.denominator == 47)
    simpleRelaxedTemporalIND = new SimpleTimeWindowTemporalIND(history1, history2, 2, false, ValidationVariant.LHS_ONLY)
    assert(simpleRelaxedTemporalIND.absoluteViolationTime == 3 + 2 + 2)
    assert(simpleRelaxedTemporalIND.denominator == 10 + 8 + 6)
    simpleRelaxedTemporalIND = new SimpleTimeWindowTemporalIND(history1, history2, 2, false, ValidationVariant.RHS_ONLY)
    assert(simpleRelaxedTemporalIND.absoluteViolationTime == 1+2)
    assert(simpleRelaxedTemporalIND.denominator == 4 + 12 + 8)
    simpleRelaxedTemporalIND = new SimpleTimeWindowTemporalIND(history1, history2, 2, false, ValidationVariant.LHS_UNION_RHS)
    assert(simpleRelaxedTemporalIND.absoluteViolationTime == 3 + 2 + 2)
    assert(simpleRelaxedTemporalIND.denominator == 23 + 8)
    simpleRelaxedTemporalIND = new SimpleTimeWindowTemporalIND(history1, history2, 2, false, ValidationVariant.LHS_INTERSECT_RHS)
    assert(simpleRelaxedTemporalIND.absoluteViolationTime == 1+2)
